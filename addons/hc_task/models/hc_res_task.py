...
class Task(models.Model):    
    ...
    priority = fields.Selection(
        ...
        selection=[
            ...
            ("normal", "Normal"), 
            ("high", "High")], 
        help="The priority of the task among other tasks of the same type.")                
    ...
    fulfillment_ids = fields.One2many(
        comodel_name="hc.task.fulfillment", 
        inverse_name="task_id", 
        string="Fulfillments", 
...
class TaskFulfillment(models.Model):    
    ...
    task_id = fields.Many2one(
        ...
        string="Task", 
        help="Task associated with this Task Fulfillment")                
    ...
    recipient_ids = fields.One2many(
        comodel_name="hc.task.fulfillment.recipient", 
...
class TaskBasedOn(models.Model):    
    ...
    based_on_string = fields.Char(
        ...
        help="String request fulfilled by this request.")               
...
class TaskFocus(models.Model):  
    ...
    focus_type = fields.Selection(
        string="Focus Type", 
        selection=[
            ("string", "String"), 
            ("Diagnostic Request", "Diagnostic Request")], 
        help="Type of what task is acting on.")              
    focus_name = fields.Char(
        string="Focus", 
        compute="_compute_focus_name", 
        store="True", 
        help="What task is acting on.")               
    focus_string = fields.Char(
        string="Focus String", 
        help="String what task is acting on.")                
    focus_diagnostic_request_id = fields.Many2one(
        comodel_name="hc.res.diagnostic.request", 
        string="Focus Diagnostic Request", 
        help="Diagnostic Request what task is acting on.")              
...
class TaskFor(models.Model):    
    ...
    for_name = fields.Char(
        string="For", 
        compute="_compute_for_name", 
        store="True", 
        help="Beneficiary of the Task.")               
    for_string = fields.Char(
        string="For String", 
        help="String beneficiary of the task.")               
    for_patient_id = fields.Many2one(
        comodel_name="hc.res.patient", 
        string="For Patient", 
        help="Patient beneficiary of the task.")             
...
class TaskFulfillmentRecipient(models.Model):   
    ...
    fulfillment_id = fields.Many2one(
        comodel_name="hc.task.fulfillment", 
        string="Fulfillment", 
        help="Fulfillment associated with this Task Fulfillment Recipient.")             
