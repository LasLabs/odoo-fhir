...
class ElementDefinitionSlicing(models.Model):    
    ...
    element_definition_id = fields.Many2one(comodel_name="hc.element.definition", string="Element Definition", help="Element Definition associated with this Element Definition Slicing.")                        
    discriminator_ids = fields.One2many(comodel_name="hc.element.definition.slicing.discriminator", inverse_name="slicing_id", string="Discriminators", help="Element values that are used to distinguish the slices.")                        
    description = fields.Text(string="Description", help="Text description of how slicing works (or not).")                        
    is_ordered = fields.Boolean(string="Ordered", help="If elements must be in same order as slices.")                        
    rules = fields.Selection(string="Rules", required="True", selection=[("closed", "Closed"), ("open", "Open"), ("openatend", "Openatend")], help="Whether additional slices are allowed or not. When the slices are ordered, profile authors can also say that additional slices are only allowed at the end.")                        
