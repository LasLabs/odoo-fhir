...
class DetectedIssue(models.Model):
    ...
    patient_id = fields.Many2one(comodel_name="hc.res.patient", string="Patient", help="Associated patient ." )
    category_id = fields.Many2one(comodel_name="hc.vs.detected.issue.category", string="Category", help="Issue Category, e.g. drug-drug, duplicate therapy, etc." )
    severity = fields.Selection(string="Severity", selection=[("high", "High"), ("moderate", "Moderate"), ("low", "Low")], help="Identifies the general type of issue identified.")
    implicated_ids = fields.One2many(comodel_name="hc.detected.issue.implicated", inverse_name="detected_issue_id", string="Implicated", help="Problem resource.")
    ...
    identifier_ids = fields.One2many(comodel_name="hc.detected.issue.identifier", inverse_name="detected_issue_id", string="Identifier", help="Identifier Unique id for the detected issue.")
    reference = fields.Char( string="Reference", help="Authority for issue.")
    mitigation_ids = fields.One2many(comodel_name="hc.detected.issue.mitigation", inverse_name="detected_issue_id", string="Mitigation", help="taken to address.")

class DetectedIssueMitigation(models.Model):
    _name = "hc.detected.issue.mitigation"
    _description = "Detected Issue Mitigation"

    detected_issue_id = fields.Many2one(comodel_name="hc.res.detected.issue", string="Detected Issue", help="Detected Issue associated with this mitigation.")
    action_id = fields.Many2one(comodel_name="hc.vs.detected.issue.mitigation.action", string="Action", required="True", help="What mitigation?")
    date = fields.Datetime(string="Date", help="Date committed.")
    author_id = fields.Many2one(comodel_name="hc.res.practitioner", string="Author", help="Who is committing?")
...
class DetectedIssueIdentifier(models.Model):
    ...
    detected_issue_id = fields.Many2one(comodel_name="hc.res.detected.issue", string="Detected Issue", help="Detected Issue associated with this detected issue identifier.")
