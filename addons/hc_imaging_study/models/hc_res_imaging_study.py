...
class ImagingStudy(models.Model):    
    ...
    uid = fields.Char(string="UID", required="True", help="Formal identifier for the study (0020,000D).")                
    accession_id = fields.Many2one(comodel_name="hc.imaging.study.accession", string="Accession", help="Accession Number (0008,0050).")                
    identifier_ids = fields.One2many(comodel_name="hc.imaging.study.identifier", inverse_name="imaging_study_id", string="Identifiers", help="Other identifiers for the study (0020,0010).")                
    availability = fields.Selection(string="Imaging Study Availability", 
        selection=[
            ("online", "Online"), ("offline", "Offline"), ("nearline", "Nearline"), 
            ("unavailable (0008,0056)", "Unavailable (0008,0056)")], 
        help="Availability of study (online, offline or nearline).")                
    modality_list_ids = fields.One2many(comodel_name="hc.imaging.study.modality.list", inverse_name="imaging_study_id", string="Modality Lists", help="All series modality if actual acquisition modalities.")                
    patient_id = fields.Many2one(comodel_name="hc.res.patient", string="Patient", required="True", help="Who the images are of.")                
    context_type = fields.Selection(string="Context Type", selection=[("Encounter", "Encounter"), ("Episode Of Care", "Episode Of Care")], help="Type of originating context.")                
    context_name = fields.Char(string="Context", compute="_compute_context_name", store="True", help="Originating context.")                
    context_encounter_id = fields.Many2one(comodel_name="hc.res.encounter", string="Context Encounter", help="Encounter originating context.")                
    context_episode_of_care_id = fields.Many2one(comodel_name="hc.res.episode.of.care", string="Context Episode Of Care", help="Episode Of Care originating context.")                
    started = fields.Datetime(string="Started Date", help="When the study was started (0008,0020)+(0008,0030).")                
    based_on_ids = fields.One2many(comodel_name="hc.imaging.study.based.on", inverse_name="imaging_study_id", string="Based On", help="Request fulfilled.")                
    referrer_id = fields.Many2one(comodel_name="hc.res.practitioner", string="Referrer", help="Referring physician (0008,0090).")                
    interpreter_id = fields.Many2one(comodel_name="hc.res.practitioner", string="Interpreter", help="Who interpreted images (0008,1060).")                
    number_of_series = fields.Integer(string="Number Of Series", required="True", help="Number of Study Related Series.")                
    number_of_instances = fields.Integer(string="Number Of Instances", required="True", help="Number of Series Related Instances.")                
    procedure_ids = fields.One2many(comodel_name="hc.imaging.study.procedure", inverse_name="imaging_study_id", string="Procedures", help="Type of procedure performed.")                
...
class ImagingStudySeries(models.Model):    
    ...
    instance_ids = fields.One2many(comodel_name="hc.imaging.study.series.instance", inverse_name="imaging_study_series_id", string="Instances", help="A single SOP instance from the series.")                
...
class ImagingStudySeriesInstance(models.Model):    
    ...
    title = fields.Char(string="Title", help="Description.")                
...
class ImagingStudyIdentifier(models.Model):    
    ...
    imaging_study_id = fields.Many2one(comodel_name="hc.res.imaging.study", string="Imaging Study", help="Imaging study associated with this imaging study identifier.")                

class ImagingStudyModalityList(models.Model):    
    _name = "hc.imaging.study.modality.list"    
    _description = "Imaging Study Modality List"        
    _inherit = ["hc.basic.association"]

    imaging_study_id = fields.Many2one(comodel_name="hc.res.imaging.study", string="Imaging Study", help="Imaging study associated with this imaging study modality list.")                
    modality_list_id = fields.Many2one(comodel_name="hc.vs.dicom.cid.29", string="Modality List", help="ModalityList associated with this imaging study modality list.")                
