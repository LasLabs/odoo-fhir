...
class ClinicalImpression(models.Model):    
    ...
    identifier_ids = fields.One2many(comodel_name="hc.clinical.impression.identifier", inverse_name="clinical_impression_id", string="Identifiers", help="Business identifier.")                
    ...
    subject_name = fields.Char(string="Subject Name", compute="compute_subject_name", required="True", help="Patient or group assessed.")                
    ...
    context_name = fields.Char(string="Context Name", compute="compute_context_name", help="Encounter or Episode created from.")                
    context_encounter_id = fields.Many2one(comodel_name="hc.res.encounter", string="Context Encounter", help="Encounter created from.")                
    context_episode_of_care_id = fields.Many2one(comodel_name="hc.res.episode of care", string="Context Episode Of Care", help="Episode Of Care created from.")                
...
class ClinicalImpressionPlan(models.Model):    
    ...
    clinical_impression_id = fields.Many2one(comodel_name="hc.res.clinical.impression", string="Clinical Impression", help="Clinical impression associated with this clinical impression plan.")                
    ...
    plan_name = fields.Char(string="Plan Name", compute="compute_plan_name", help="Plan of action after assessment.")                
    plan_appointment_id = fields.Many2one(comodel_name="hc.res.appointment", string="Plan Appointment", help="Appointment plan of action after assessment.")                
    plan_communication_request_id = fields.Many2one(comodel_name="hc.res.communication request", string="Plan Communication Request", help="Communication Request plan of action after assessment.")                
    plan_device_use_request_id = fields.Many2one(comodel_name="hc.res.device use request", string="Plan Device Use Request", help="Device Use Request plan of action after assessment.")                
    plan_diagnostic_order_id = fields.Many2one(comodel_name="hc.res.diagnostic order", string="Plan Diagnostic Order", help="Diagnostic Order plan of action after assessment.")                
    plan_medication_order_id = fields.Many2one(comodel_name="hc.res.medication order", string="Plan Medication Order", help="Medication Order plan of action after assessment.")                
    plan_nutrition_order_id = fields.Many2one(comodel_name="hc.res.nutrition order", string="Plan Nutrition Order", help="Nutrition Order plan of action after assessment.")                
    plan_order_id = fields.Many2one(comodel_name="hc.res.order", string="Plan Order", help="Order plan of action after assessment.")                
    plan_procedure_request_id = fields.Many2one(comodel_name="hc.res.procedure request", string="Plan Procedure Request", help="Procedure Request plan of action after assessment.")                
    plan_process_request_id = fields.Many2one(comodel_name="hc.res.process request", string="Plan Process Request", help="Process Request plan of action after assessment.")                
    plan_referral_request_id = fields.Many2one(comodel_name="hc.res.referral request", string="Plan Referral Request", help="Referral Request plan of action after assessment.")                
    plan_supply_request_id = fields.Many2one(comodel_name="hc.res.supply request", string="Plan Supply Request", help="Supply Request plan of action after assessment.")                
    plan_vision_prescription_id = fields.Many2one(comodel_name="hc.res.vision prescription", string="Plan Vision Prescription", help="Vision Prescription of the plan of action after assessment.")                
...
class ClinicalImpressionAction(models.Model):    
    ...
    action_name = fields.Char(string="Action Name", compute="compute_action_name", help="Actions taken during assessment.")                
    action_diagnostic_order_id = fields.Many2one(comodel_name="hc.res.diagnostic order", string="Action Diagnostic Order", help="Diagnostic Order actions taken during assessment.")                
    action_procedure_request_id = fields.Many2one(comodel_name="hc.res.procedure request", string="Action Procedure Request", help="Procedure Request actions taken during assessment.")                
    action_procedure_id = fields.Many2one(comodel_name="hc.res.procedure", string="Action Procedure", help="Procedure actions taken during assessment.")                
    action_medication_order_id = fields.Many2one(comodel_name="hc.res.medication order", string="Action Medication Order", help="Medication Order actions taken during assessment.")                
    action_diagnostic_order_id = fields.Many2one(comodel_name="hc.res.diagnostic order", string="Action Diagnostic Order", help="Diagnostic Order actions taken during assessment.")                
    action_nutrition_order_id = fields.Many2one(comodel_name="hc.res.nutrition order", string="Action Nutrition Order", help="Nutrition Order actions taken during assessment.")                
    action_supply_request_id = fields.Many2one(comodel_name="hc.res.supply request", string="Action Supply Request", help="Supply Request actions taken during assessment.")                
    action_appointment_id = fields.Many2one(comodel_name="hc.res.appointment", string="Action Appointment", help="Appointment actions taken during assessment.")                
...
class ClinicalImpressionInvestigation(models.Model):    
    ...
    item_name = fields.Char(string="Item Name", compute="compute_item_name", help="Record of a specific investigation.")                
    item_observation_id = fields.Many2one(comodel_name="hc.res.observation", string="Item Observation", help="Observation record of a specific investigation.")                
    item_questionnaire_response_id = fields.Many2one(comodel_name="hc.res.questionnaire response", string="Item Questionnaire Response", help="Questionnaire Response record of a specific investigation.")                
    item_family_member_history_id = fields.Many2one(comodel_name="hc.res.family member history", string="Item Family Member History", help="Family Member History record of a specific investigation.")                
    item_diagnostic_report_id = fields.Many2one(comodel_name="hc.res.diagnostic report", string="Item Diagnostic Report", help="Diagnostic Report record of a specific investigation.")                
    item_risk_assessment_id = fields.Many2one(comodel_name="hc.res.risk assessment", string="Item Risk Assessment", help="Risk Assessment record of a specific investigation.")                
    item_imaging_study_id = fields.Many2one(comodel_name="hc.res.imaging study", string="Item Imaging Study", help="Imaging Study record of a specific investigation.")                
...
class ClinicalImpressionFinding(models.Model):    
    ...
    item_type = fields.Selection(string="Finding Item Type", selection=[("Condition", "Condition"), ("Observation", "Observation")], help="Type of what was found.")                
    item_name = fields.Char(string="Item Name", compute="compute_item_name", required="True", help="What was found.")                
    item_code_id = fields.Many2one(comodel_name="hc.vs.condition.code", string="Item Code", required="True", help="What was found")                
    item_condition_id = fields.Many2one(comodel_name="hc.res.condition", string="Item Condition", required="True", help="Condition what was found.")                
    item_observation_id = fields.Many2one(comodel_name="hc.res.observation", string="Item Observation", required="True", help="Observation what was found.")                
    cause = fields.Char(string="Cause", help="Which investigations support finding.")                
...
class ClinicalImpressionPrognosisReference(models.Model):    
    ...
    risk_assessment_id = fields.Many2one(comodel_name="hc.res.risk.assessment", string="Risk Assessment", help="Risk assessment associated with this clinical impression prognosis reference.")                
...
class ClinicalImpressionProblem(models.Model):    
    ...
    problem_name = fields.Char(string="Problem Name", compute="compute_problem_name", help="General assessment of patient state.")                
    problem_condition_id = fields.Many2one(comodel_name="hc.res.condition", string="Problem Condition", help="Condition general assessment of patient state.")                
    problem_allergy_intolerance_id = fields.Many2one(comodel_name="hc.res.allergy intolerance", string="Problem Allergy Intolerance", help="Allergy Intolerance general assessment of patient state.")                
...
class ClinicalImpressionSet(models.Model):    
    _name = "hc.vs.clinical.impression.set"    
    _description = "Clinical Impression Set"        
    _inherit = ["hc.value.set.contains"]
