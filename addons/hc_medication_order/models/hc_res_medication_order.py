...
class MedicationOrder(models.Model):    
    ...
    prescriber_id = fields.Many2one(comodel_name="hc.res.practitioner", string="Prescriber", help="Who ordered the medication(s).")                
    reason_code_ids = fields.One2many(comodel_name="hc.medication.order.reason.code", inverse_name="medication_order_id", string="Reason Codes", help="Reason or indication for writing the prescription.")                
...
class MedicationOrderDosageInstruction(models.Model):    
    ...
    dose_uom_id = fields.Many2one(comodel_name="product.uom", string="Dose UOM", help="Dose unit of measure.")                
    max_dose_quantity = fields.Float(string="Maximum Dose Quantity", help="Numerator value of upper limit on medication per unit of time.")                
    max_dose_period = fields.Float(string="Maximum Dose Period", help="Denominator value of upper limit on medication per unit of time.")                
    ...
    max_dose_per_administration = fields.Float( help="Upper limit on medication per unit of time.")                
    ...
    dose_rate_uom_id = fields.Many2one(comodel_name="product.uom", string="Dose Rate UOM", help="Amount of medication per unit of time unit of measure.")                
...
class MedicationOrderDispenseRequest(models.Model):    
    ...
    medication_order_id = fields.Many2one(comodel_name="hc.res.medication.order", string="Medication Order", required="True", help="Medication order associated with this dispense request.")                
    start_date = fields.Datetime(string="Start Date", help="Start of the time period supply is authorized for.")                
    end_date = fields.Datetime(string="End Date", help="End of the time period supply is authorized for.")                
    number_repeats_allowed = fields.Integer(string="Number Repeats Allowed", help="# of refills authorized.")                
    ...
    expected_supply_duration = fields.Float(string="Expected Supply Duration", help="Days supply per dispense.")                
    expected_supply_duration_uom_id = fields.Many2one(comodel_name="product.uom", string="Expected Supply Duration UOM", help="Days supply per dispense unit of measure." )                
...
class MedicationOrderSubstitution(models.Model):    
    ...
    medication_order_id = fields.Many2one(comodel_name="hc.res.medication.order", string="Medication Order", help="Medication order associated with this substitution.")                
    allowed = fields.Boolean(string="Substitution Allowed", required="True", help="Whether substitution is allowed or not.")                
    reason_id = fields.Many2one(comodel_name="hc.vs.substance.admin.substitution.reason", string="Reason", help="Why should substitution (not) be made.")                
...
class MedicationOrderIdentifier(models.Model):    
    ...
    medication_order_id = fields.Many2one(comodel_name="hc.res.medication.order", string="Medication Order", help="Medication order associated with this medication order identifier.")                
...
class MedicationOrderReasonCode(models.Model):    
    ...
    medication_order_id = fields.Many2one(comodel_name="hc.res.medication.order", string="Medication Order", help="Medication order associated with this medication order reason code.")                
    reason_code_id = fields.Many2one(comodel_name="hc.vs.condition.code", string="Reason Code", help="Reason Code associated with this medication order reason code.")                
...
class MedicationOrderReasonReference(models.Model):    
    ...
    medication_order_id = fields.Many2one(comodel_name="hc.res.medication.order", string="Medication Order", help="Medication order associated with this medication order reason reference.")                
    reason_reference_id = fields.Many2one(comodel_name="hc.res.condition", string="Reason Reference", help="Reason Reference associated with this medication order reason reference.")                
...
class MedicationOrderNote(models.Model):    
    ...
    medication_order_id = fields.Many2one(comodel_name="hc.res.medication.order", string="Medication Order", help="Medication order associated with this medication order note.")                
